...
def main():
    parser = argparse.ArgumentParser(
        ...
        description="递归文件/内容查找工具（支持指定目录+排除目录+进度条）\n"
                    "使用示例:\n\n"
    ...
    parser.add_argument("--mode", required=True, choices=["file", "content"], help="查找模式：file=查文件，content=查内容")
    parser.add_argument("--root", required=True, help="根目录，例如 D:\\")
    parser.add_argument("--file", help="要查找的文件名（mode=file 时使用）")
    parser.add_argument("--content", help="要查找的文件内容（mode=content 时使用）")
    parser.add_argument("--folders", nargs="+", required=True, help="只在这些文件夹下查找，例如 src release dist")
    parser.add_argument("--exclude", nargs="+", default=["node_modules", ".git", "venv", "test"], help="要排除的目录")

    args = parser.parse_args()

    print("=" * 60)
    print(f"📂 根目录：{args.root}")
    print(f"📁 仅查找目录：{args.folders}")
    print(f"🚫 排除目录：{args.exclude}")
...
# 查找内容：D 盘下 src 里包含 xfs_token 的文件
# python search_tool.py --mode content --root D:\ --content xfs_token --folders src