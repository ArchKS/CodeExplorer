...
(function() {
    ...
    async function collectAllData() {
        ...
        requestAnimationFrame(() => {
            ...
            // 优先级排序：营业总收入 > 营业收入 > 包含“收入”字样的行
            let targetRow = allRows.find(r => {
                const label = r.querySelector('td.white, th.white')?.textContent || '';
                return label.trim() === '营业总收入' || label.includes('营业总收入(元)') || label.includes('收入');
            });

            if (!targetRow) {
                targetRow = allRows.find(r => {
                    const label = r.querySelector('td.white, th.white')?.textContent || '';
                    return label.includes('营业总收入') || label.includes('营业收入') || label.includes('收入');
                });
            }
